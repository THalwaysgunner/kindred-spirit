   ...
   useEffect(() => {
      const fetchData = async () => {
         ...
         try {
            ...
            // D. Fetch Interviews using job_function (category like ENGINEERING)
            const interviewsData = await GlassdoorService.getInterviews(companyId, 1, jobCategory);
            const finalTerm = jobCategory;
   ...
   const mapInterviews = (rawList: any[]): InterviewReview[] => {
      return rawList.map((int: any) => {
         ...
         if (int.difficulty) {
            ...
            if (d === 'difficult' || d === 'very_difficult') { difficulty = 'Difficult'; difficultyScore = 4; }
         ...
         // Questions - API returns array of strings directly
         const questions = (int.questions || []).filter((q: string) => q && q.length > 0);

         // Format date from review_datetime
         let formattedDate = 'Recent';
         if (int.review_datetime) {
            formattedDate = new Date(int.review_datetime).toLocaleDateString();
         }
         ...
         return {
            ...
            applicationProcess: int.process_description || '',
            questions: questions,
                     ...
                     >
                        ...
                        <div className="space-y-3">
                           ...
                           <div className="flex justify-between items-center text-sm">
                              ...
                              <span className={`font-bold px-2 py-0.5 rounded text-xs ${app.status === 'Offer' ? 'bg-green-100 text-green-700' :
               ...
               <div className="flex gap-8 border-b border-slate-200 dark:border-slate-700 mt-6 overflow-x-auto">
                  {['overview', 'interviews', 'salaries', 'reviews'].map((tab) => (
                     <button
                        ...
                        className={`pb-3 text-sm font-bold capitalize transition-colors relative whitespace-nowrap ${activeTab === tab
